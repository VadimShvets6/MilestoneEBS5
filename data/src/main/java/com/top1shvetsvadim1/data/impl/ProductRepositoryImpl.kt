...
import com.top1shvetsvadim1.data.ProductPagingSource
...
class ProductRepositoryImpl @Inject constructor(
    ...
    private fun getProductPager(filter: Filters) = Pager(
        PagingConfig(PAGE_SIZE),
        pagingSourceFactory = { ProductPagingSource(apiService, filter) }
    ).flow

    override suspend fun getProductList(filter: Filters): Flow<List<ProductItem>> {
       //return getProductPager(filter)
       return flowOf(apiService.getProductList(page = 1, ordering = filter.name.lowercase()).results)
//        Log.d("Test", "Repo: $filter")
//        return combine(
//            flowOf(apiService.getProductList(page = 1, ordering = filter.name.lowercase()).results),
//            //getFavoriteList(),
//            getCartList()
...
//    }
    ...
    override suspend fun getItemById(id: Int): Flow<ProductItem> {
